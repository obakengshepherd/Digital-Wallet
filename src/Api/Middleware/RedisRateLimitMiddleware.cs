...
{
    // ── Digital Wallet ────────────────────────────────────────────────────────
    public static IEnumerable<RateLimitRule> WalletPolicies() =>
    [
        // POST /wallets/transfer — strictest limit (financial write)
        new RateLimitRule
        {
            PolicyName = "wallet-transfer",
            Limit      = 10,
            Window     = TimeSpan.FromMinutes(1),
            Paths      = ["/api/v1/wallets/transfer"],
            Methods    = ["POST"]
        },
        // POST /wallets/{id}/deposit — moderate write limit
        new RateLimitRule
        {
            PolicyName = "wallet-deposit",
            Limit      = 20,
            Window     = TimeSpan.FromMinutes(1),
            Paths      = ["/api/v1/wallets"],
            Methods    = ["POST"]
        },
        // GET /wallets/{id}/transactions — generous read limit
        new RateLimitRule
        {
            PolicyName = "wallet-reads",
            Limit      = 200,
            Window     = TimeSpan.FromMinutes(1),
            Paths      = ["/api/v1/wallets"],
            Methods    = ["GET"]
        },
        // Default authenticated limit — all other endpoints
        new RateLimitRule
        {
